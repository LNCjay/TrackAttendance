...
const collegePrograms = {
    ...
    "IHK": [
        "Bachelor of Science in Physical Education (Boni Campus)" // Added quotes
    ]
};
...
const DB_URL = "https://attendance-6176a-default-rtdb.asia-southeast1.firebasedatabase.app/";

// Add these constants at the top of your file
const TWILIO_ACCOUNT_SID = 'AC929c019a08252bbc4c7bc2166aa41de9';
const TWILIO_AUTH_TOKEN = '21ccf77fe9e097c9df0851113a104b35';
const TWILIO_PHONE_NUMBER = '+13412301471';
...
async function sendSMS(to, message) {
    const instanceId = '9eeb01c5-4126-4cca-8ba7-146169840e9e'; // your actual instance UUID
    const apiKey = '179|QubTBzpKVu8PD77pBFEuL0cIoBnw1zRqcZylX6vr43d94727';
    const requestId = crypto.randomUUID(); // optional

    try {
        const response = await fetch(`https://app.hypersender.com/api/sms/v1/${instanceId}/send-message`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
        ...
        if (!response.ok) {
            console.error('HyperSender API error:', data);
...
function formatScheduleForDisplay(schedule) {
    return formatScheduleForTable(schedule);
}
...
async function searchStudent() {
    ...
    try {
        ...
        scheduleDisplay.innerHTML = formatScheduleForDisplay(student.schedule);
    ...
    let tableHtml = `
        <div class="scrollable-table-container">
            <table class="schedule-table">
                <thead>
                    <tr><th>Day</th><th>Subject & Time</th></tr>
                </thead>
                <tbody>
    `;

    // Loop through each day and format the schedule accordingly
    daysOfWeek.forEach(day => {
        const daySchedule = schedule[day];

        if (daySchedule && daySchedule !== timeOptionVacant) {
            isAllVacant = false;
            let scheduleText = '';

            // Format the schedule for each day
            if (Array.isArray(daySchedule)) {
                scheduleText = daySchedule.map(e =>
                    `${e.subject.substring(0, 10)} (${e.time})`
                ).join(', ');
            } else if (typeof daySchedule === 'object' && daySchedule.subject) {
                scheduleText = `${daySchedule.subject.substring(0, 10)} (${daySchedule.time})`;
            } else if (typeof daySchedule === 'string') {
                scheduleText = daySchedule;
            }

            tableHtml += `<tr><td>${day}</td><td>${scheduleText}</td></tr>`;
        }
    });

    tableHtml += '</tbody></table></div>';

    // If all days are vacant, show a special message
    let detailsHtml = '<details>';
    detailsHtml += '<summary>View Schedule</summary>';

    if (isAllVacant) {
        detailsHtml += '<span class="vacant-text">All Days Vacant</span>';
    } else {
        detailsHtml += `<div class="full-schedule-content">${tableHtml}</div>`;
    }
...
async function loadStudentRecords() {
    ...
    filteredStudents.forEach(([studentNumber, student]) => {
        ...
        row.innerHTML = `
            ...
            <td>${student.year}</td>
            <td class="schedule-column">${scheduleDisplay}</td>
            <td>
...
function updateProgramDropdown(college, selectedProgram) {
   ...
   const programs = {
        CEA: [ 'Bachelor of Science in Mechanical Engineering',
                    'Bachelor of Science in Civil Engineering',
                    'Bachelor of Science in Electrical Engineering',
                    'Bachelor of Science in Electronics Engineering',
                    'Bachelor of Science in Computer Engineering',
                ...
                CED: [ // Changed COE to CED
                    ...
                    'Bachelor or Technical-Vocational Teacher Education Major in Electronics Technology',
                    'Bachelor or Technical-Vocational Teacher Education Major in Welding and Fabrications Technology'
                ], // Removed extra comma and bracket
                IHK: [
                    'Bachelor of Science in Physical Education (Boni Campus)' // Added quotes
                ]
    };
